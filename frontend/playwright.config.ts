  ...
  webServer: [
    {
      ...
      command: [
        ...
        "&& (cd server && DATABASE_URL='sqlite+aiosqlite:///e2e_test.db' uv run alembic upgrade head)",
        `&& DATABASE_URL='sqlite+aiosqlite:///server/e2e_test.db' WAYGATE_ADMIN_USER='${ADMIN_USERNAME}' WAYGATE_ADMIN_PASSWORD='${ADMIN_PASSWORD}' uv run granian --interface asgi --http 1 server.main:app --host 127.0.0.1 --port 8000`,
      ].join(" "),
