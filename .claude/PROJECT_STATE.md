...
| — | **Test-AWG-server 2026-05-06**: `scripts/setup-test-awg-server.sh` обновлён на полные AWG 2.0 параметры из `AWG_PARAMS.txt` (S3/S4, I2-I5, H-ranges); параметры вынесены в переменные для гарантии симметрии server/client. **Корень проблемы из SESSION_2026_05_04 найден**: на Ubuntu 24.04 + Docker 28+ команды `iptables` попадают в shadow-chain от iptables-nft-compat и НЕ срабатывают (policy=drop в реальной Docker-managed `chain ip filter FORWARD`). Решение — писать через `nft` напрямую. Зафиксировано в `ROUTING_ARCHITECTURE.md` + `SESSION_2026_05_06.md` + memory. | ✅ |
| — | **Системный fix apply-flow 2026-05-06 (Phase A/B/C)**: `agent/awg_clients.deploy_client` теперь `_wait_for_iface` после `docker run -d` (закрывает race "Cannot find device"); `routing.apply_rules` retry x2 для netns-switch + persistent `last-apply.json`; `main.py` честный counter + `AgentStatus.last_apply_errors/succeeded`; server `_apply_helper.run_full_apply` extracted; `healthcheck._auto_reapply_if_needed` авто-повтор при OFFLINE→ONLINE с throttle 60s; integration-тесты переписаны на `nft list` (закрывает NFT-1 — ловит shadow-chain). 209 unit + 14 integration зелёные. | ✅ |
| — | **Apply-flow устаканивание 2026-05-06 (NFT-3/5/6/7)**: после Phase D verification на yandex VM выявились грабли которые требовали ручных команд на VM. Закрыто в коде: `shared/awg_config.py` default MTU=1280 для AWG-iface (двойной туннель не фрагментируется); `provisioner/steps.py::install_deps` ставит `update-alternatives --set iptables iptables-nft` (на свежей Ubuntu 24.04 alternative по дефолту legacy → правила agent'а не действуют); `agent/routing.py::_recover_mangle_if_incompatible` авто-flush mangle table если она стала "incompatible" для iptables-nft compat; `scripts/setup-test-awg-server.sh` PostUp/PostDown через `nft` в отдельной table `ip waygate-test` (работает на любой alternative). 215 unit + 10 integration зелёные. Закрыты NFT-3/5/6/7 в backlog. | ✅ |
| — | **Финал-полировка 2026-05-06 (NFT-4/8a + 0a-future + ssh-heredoc)**: agent 0.2.32 + server 0.1.2. **NFT-4**: `_count_masquerades_per_iface` с fallback на `nft list` (когда `iptables -S` падает с incompatible) + dedup-loop в `_apply_rules_in_scope_family` чтобы каждый apply не плодил новый MASQ. **NFT-8a**: `awg_clients.enforce_existing_iface_mtu` startup-action для existing iface'ов (deploy'нутых до 0.2.32 с MTU=1420) — без перезапуска контейнера. **0a-future**: cleanup port-specific bypass'ов (теперь только `addrtype LOCAL` + `ESTABLISHED` в PREROUTING — addrtype покрывает SSH/agent/handshake'и одной строкой). **ssh-heredoc**: `provisioner/ssh.py::write_file` нормализует trailing-newline (был extra `\n` если content end'ит на newline). 218 unit + 16 integration зелёные. Закрыты NFT-2/4 (NFT-2 = NFT-7 same scope), NFT-8 partial (8a runtime fix; full UI-config — NFT-8b backlog). | ✅ |

## Тестирование

- **Backend (unit/api)**: `cd backend && uv run pytest` — **218 passed** (после 2026-05-06: +5 apply-flow + 4 mangle-recovery + 2 wait-iface + 3 auto-reapply healthcheck + 3 mtu/legacy-config + 2 dedupe-MASQ/nft-fallback + 2 enforce-mtu).
- **Backend (integration)**: `cd backend && uv run pytest -m integration` — **16 passed** (~60-90 сек, поднимает реальный `--privileged` agent-контейнер с ipset/iptables/**nftables**/dnsmasq + Ubuntu sshd-контейнер; гоняет HTTP к живому granian'у и SSH-flow против реального OpenSSH'а). Existing 4 переписаны на `nft list chain` (закрывает shadow-chain bug); 3 новых на recovery race / status reporting / orphan cleanup. ssh-heredoc trailing-newline тесты теперь зелёные (NFT-fix через `write_file` normalize). Запускать локально с docker daemon (или в CI на release-tags).
