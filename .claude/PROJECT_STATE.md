...
- **Backend (unit/api)**: `cd backend && uv run pytest` — **209 passed** (+5 новых после fix apply-flow: 2 _wait_for_iface, 3 auto-reapply healthcheck).
- **Backend (integration)**: `cd backend && uv run pytest -m integration` — **14 passed** (~60-90 сек, поднимает реальный `--privileged` agent-контейнер с ipset/iptables/**nftables**/dnsmasq + Ubuntu sshd-контейнер; гоняет HTTP к живому granian'у и SSH-flow против реального OpenSSH'а). 4 переписанных теста с `iptables -S` на `nft list chain` + 3 новых на recovery race / status reporting / orphan cleanup. 2 ssh_integration теста (heredoc-quotes) — known pre-existing failure, не блокирует. По умолчанию выключены через `addopts`. Запускать локально с docker daemon (или в CI на release-tags).
