...
| — | **Системный fix apply-flow 2026-05-06 (Phase A/B/C)**: `agent/awg_clients.deploy_client` теперь `_wait_for_iface` после `docker run -d` (закрывает race "Cannot find device"); `routing.apply_rules` retry x2 для netns-switch + persistent `last-apply.json`; `main.py` честный counter + `AgentStatus.last_apply_errors/succeeded`; server `_apply_helper.run_full_apply` extracted; `healthcheck._auto_reapply_if_needed` авто-повтор при OFFLINE→ONLINE с throttle 60s; integration-тесты переписаны на `nft list` (закрывает NFT-1 — ловит shadow-chain). 209 unit + 14 integration зелёные. **Open:** Phase D — verify на yandex VM после деплоя агента. NFT-2 (переписать setup-test-awg-server.sh на nft) — отдельным следующим пакетом. | ✅ |
